...
def test_gpt_connection():
    ...
    try:
        ...
        # 初始化客户端
        client = OpenAI(api_key=api_key, base_url=config.get_openai_api_base())
...
def test_reddit_summary():
    ...
    try:
        ...
        client = OpenAI(api_key=config.get_openai_api_key(), base_url=config.get_openai_api_base())
...
4. 不超过50字"""
        ...
        response = client.chat.completions.create(
            model=config.get_openai_model(),
            messages=[{"role": "system", "content": "你是一个专业的新闻总结助手。"}, {"role": "user", "content": prompt}],
            max_tokens=100,
            temperature=0.3,
        )
